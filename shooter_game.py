...
class Player(GameSprite):
    def update(self):
        ...
        if keys[K_RIGHT] and self.rect.x < win_width - 80:
            self.rect.x += self.speed
    def fire(self):
        bullet = Bullet(img_bullet,self.rect.centerx, self.rect.top, 15, 20, randint(-100, -10))
        bullets.add(bullet)
...
background = transform.scale(image.load(img_back), (win_width,
win_height))
bullets = sprite.Group()
player = Player(img_hero, 5, 400, 100, 80, 15)
...
for i in range(1, 3):
    asteroid = Enemy(img_ast, randint(80, win_width - 80),
     -40, 80, 50, randint(3, 8))
...
while run:
    ...
    if not finish:
        ...
        text = font1.render("Счет: " + str(score),
         1, (255, 255, 255))
